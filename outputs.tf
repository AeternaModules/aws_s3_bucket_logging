...
output "s3_bucket_loggings_target_object_key_format" {
  ...
  value       = { for k, v in aws_s3_bucket_logging.s3_bucket_loggings : k => one(v.target_object_key_format) if v.target_object_key_format != null && length(v.target_object_key_format) > 0 }
}
